...
class HasMany extends Association {
	...
	function push($args, &$source) {
		foreach ($args as $object) {
			if(($source->is_new_record() || $object->is_new_record()) && $this->options['through'])
		...
		if(!is_array($this->value) || $force){
			...
			try{
				if(!$this->options['through']){
					$collection = call_user_func_array(array($this->dest_class, 'find'), array('all', array('conditions' => "{$this->foreign_key} = ".$source->{$source->get_primary_key()})));
				}else{
					
					// TODO: $this->options['through'] is not necessarily the table name
					$collection = call_user_func_array(array($this->dest_class, 'find'),array('all',array('include' => $this->options['through'],
	...
	function get_ids(&$source, $force='false') {
	  ...
	  foreach ($objects as $object) $ids[] = $object->{$object->get_primary_key()};
	  return $ids;
	}
	...
	function set_ids($ids, &$source) {
  	...
  	// Break up the relationship $objects = array of $objects that are related but should no longer be $source = source object 
	...
	function join() {
	    ...
	    $columns = $dest_inst->get_columns($dest_inst);
	...
	function populate_from_find($attributes) {
  	...
  	function needs_saving() {
		if (!is_array($this->value)){
			...
			foreach ($this->value as $val){
				if ($val->is_modified() || $val->is_new_record()) return true;
			}
		}
  	...
  	function save_as_needed($source) {
		foreach ($this->value as $object){
			if($object->is_modified() || $object->is_new_record()){
				if (!$this->options['through'])	$object->{$this->foreign_key} = $source->{$source->get_primary_key()};
				$object->save();
			}
		}
	}
